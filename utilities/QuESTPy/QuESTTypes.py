"""
Module defining various QuEST types for use with python interface
Contains:
- Precision definitions
- Parsing routines for strings to QuEST types
- Functions for generating random complex structures
"""

from ctypes import (Structure, POINTER, byref,
                    c_float, c_double, c_longdouble, c_int, c_long, c_longlong, c_char_p)
import random
import math

from .QuESTBase import (QuESTLib,
                        compArrKeyWarning, nQubitsWarning, argWarningGen, argWarning, typeWarning)
...
class QASMLogger(Structure):
    """ Class to handle the QuEST QASM logging routines """
...
class ComplexArray(Structure):
    """ Class to handle arbitrary Complex Arrays """
    ...
    def __getitem__(self, key):
        if isinstance(key, slice):
            ...
            stop = key.stop
            step = key.step  or 1
            out = [Complex(self.real[i], self.imag[i]) for i in range(start, stop, step)]
        elif isinstance(key, int):
            out = Complex(self.real[key], self.imag[key])
        else:
            raise IndexError(compArrKeyWarning.format(type(key).__name__))
        return out

class Complex(Structure):
    """ Standard QuEST complex number """
    _fields_ = [("real", qreal),
                ("imag", qreal)]

    if qreal is c_float:
        __repr__ = lambda self: "({:9.7f},{:9.7f})".format(self.real, self.imag)
    elif qreal is c_double:
        __repr__ = lambda self: "({:15.13f},{:15.13f})".format(self.real, self.imag)
    elif qreal is c_longdouble:
        __repr__ = lambda self: "({:20.18f},{:20.18f})".format(self.real, self.imag)
...
class Qureg(Structure):
    """ QuEST qubit register class """
    def __str__(self):
        if not self._size_warn():
            return ""
        retString = ""
    ...
    def __getitem__(self, key):
        if isinstance(key, slice):
            ...
            stop = key.stop
            step = key.step  or 1
            if not self._size_warn((stop-start)//step):
                return None

        return self.stateVec[key]

    def _size_est(self, numElem):
...
class QuESTEnv(Structure):
    """ QuEST Environment structure """
    _fields_ = [("rank", c_int), ("numRanks", c_int)]

def stringToList(arg):
    """ Turn a comma-separated string into a list of floats """
    if not isinstance(arg, str):
        raise TypeError(argWarningGen.format('stringToList', str.__name__, type(arg).__name__))
    arg = arg.strip(',').split(',')
    try:
...
def stringToComplex(arg):
    """ Turn a comma-separated, round-bracketed pair of values into Complex """
    if not isinstance(arg, str):
        raise TypeError(argWarningGen.format('stringToList', str.__name__, type(arg).__name__))
    arg = arg.lstrip('(').rstrip(')')
    return Complex(* list(map(float, arg.split(','))))

def argVector(arg):
    """ Turn a list, tuple or string into a vector type """
    if isinstance(arg, Vector):
        ...
        raise TypeError(argWarningGen.format('argVector', 'str, tuple or list', type(arg).__name__))

def argComplexMatrix2(arg):
    """ Turn a list, tuple or string into a ComplexMatrix2 type """
    if isinstance(arg, ComplexMatrix2):
        return arg
    ...
    if isinstance(arg, (tuple, list)):
        ...
        raise TypeError(argWarningGen.format('argComplexMatrix2', 'str, tuple or list', type(arg).__name__))
    elements = []
    for i in range(0, len(vals), 2):
        elements.append(Complex(vals[i], vals[i+1]))
    if len(elements) != 4:
        raise TypeError(argWarningGen.format('argComplexMatrix2', '4 arguments', len(elements)+" arguments"))
    return ComplexMatrix2(*elements)

def argComplex(arg):
    """ Turn a list, tuple or string into a Complex type """
    ...
    raise TypeError(argWarningGen.format('argComplex', 'str, tuple or list', type(arg).__name__))

def argComplexArray(arg):
    """ Turn a list, tuple or string into a ComplexArray type """

    if isinstance(arg, ComplexArray):
        return arg
    ...
    if isinstance(arg, (list, tuple)):
        if all(map(lambda x: isinstance(x, Complex))):
            ...
            vals = arg
        else: raise TypeError(argWarningGen.format('argComplexArray', 'list of float/Complex', type(arg).__name__))
    elif isinstance(arg, str):
        vals = stringToList(arg)
    else:
        raise TypeError(argWarningGen.format('argComplexArray', 'str, tuple or list', type(arg).__name__))
...
def argPointerQreal(arg):
    """ Turn a list, tuple or string into a pointer to a real """
    ...
    if isinstance(arg, (list, tuple)):
        ...
        return arg
    else: raise TypeError(argWarningGen.format('argVector', 'str, float, tuple or list', type(arg).__name__))

def argPointerInt(arg):
    """ Turn a list, tuple or string into a pointer to an int """
    if isinstance(arg, str):
...
def argPointerLongInt(arg):
    """ Turn a list, tuple or string into a pointer to a long int """
...
class QuESTTestee:
    ...
    funcsList = property(lambda: QuESTTestee._funcsList)
    funcsDict = property(lambda: QuESTTestee._funcsDict)

    def __init__(self, funcname=None, retType=None, argType=None, defArg=None, denMat=None):
        if argType is None:
            argType = []
        if defArg is None:
            argType = []
    ...
    def __call__(self, *argsList):
        ...
        if (not specArg and self.nArgs != 0) or (self.nArgs == 0):
    ...
    get_func = lambda name: QuESTTestee._funcsDict[name]
...
def list_funcnames():
    """ Return a list of the names of functions """
...
def rand_norm_comp():
    """ Return a random normalised complex number """
...
def rand_norm_comp_pair():
    """ Return a pair of normalised complex numbers """
