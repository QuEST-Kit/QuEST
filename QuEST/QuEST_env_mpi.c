...
REAL calcTotalProbability(MultiQubit multiQubit)
{
    ...
    //   point operation overhead. For more details see
    ...
    if (multiQubit.numChunks > 1)
        MPI_Allreduce(&pTotal, &allRankTotals, 1, MPI_QuEST_REAL, MPI_SUM,
            MPI_COMM_WORLD);
    else
...
/** Returns whether a given chunk in position chunkId is in the upper or lower
 half of a block.
 *
 ...
 * @return 1: chunk is in upper half of block, 0: chunk is in lower half of
 block
 */
 ...
 * @param[in] chunkIsUpper 1: chunk is in upper half of block, 0: chunk is in
 lower half
 *
 * @param[out] rot1, rot2 rotation values to use, allocated for upper/lower such
 that
 * @verbatim
...
void exchangeStateVectors(MultiQubit multiQubit, int pairRank)
{
    ...
    // Multiple messages are required as MPI uses int rather than long long int
    // for count For openmpi, messages are further restricted to 2GB in size -- do
    // this for all cases to be safe
    long long int maxMessageCount = 1LL << 29;
    if (sizeof(REAL) == 8)
        maxMessageCount = (1LL << 28);
    else if (sizeof(REAL) == 16)
        maxMessageCount = (1LL << 27);

    if (multiQubit.numAmps < maxMessageCount)
        maxMessageCount = multiQubit.numAmps;
    ...
    for (i = 0; i < numMessages; i++) {
        ...
        MPI_Sendrecv(&multiQubit.stateVec.real[offset], maxMessageCount,
            MPI_QuEST_REAL, pairRank, TAG,
            &multiQubit.pairStateVec.real[offset], maxMessageCount,
            MPI_QuEST_REAL, pairRank, TAG, MPI_COMM_WORLD, &status);
        // printf("rank: %d err: %d\n", multiQubit.rank, err);
        MPI_Sendrecv(&multiQubit.stateVec.imag[offset], maxMessageCount,
            MPI_QuEST_REAL, pairRank, TAG,
            &multiQubit.pairStateVec.imag[offset], maxMessageCount,
            MPI_QuEST_REAL, pairRank, TAG, MPI_COMM_WORLD, &status);
    }
}
...
void compactUnitary(MultiQubit multiQubit,
    ...
    Complex beta)
{
    QuESTAssert(targetQubit >= 0 && targetQubit < multiQubit.numQubits, 1,
        __func__);
    ...
    if (useLocalDataOnly) {
        ...
        pairRank = getChunkPairId(rankIsUpper, multiQubit.chunkId,
            multiQubit.numAmps, targetQubit);
    ...
    Complex beta)
{
    QuESTAssert(targetQubit >= 0 && targetQubit < multiQubit.numQubits, 1,
        __func__);
    QuESTAssert(controlQubit >= 0 && controlQubit < multiQubit.numQubits, 2,
        __func__);
    ...
    if (useLocalDataOnly) {
        // all values required to update state vector lie in this rank
        controlledCompactUnitaryLocal(multiQubit, controlQubit, targetQubit, alpha,
            beta);
    } else {
        // need to get corresponding chunk of state vector from other rank
        rankIsUpper = chunkIsUpper(multiQubit.chunkId, multiQubit.numAmps, targetQubit);
        ...
        if (rankIsUpper) {
            controlledCompactUnitaryDistributed(multiQubit, controlQubit, targetQubit,
                rot1, rot2,
                multiQubit.stateVec, // upper
                multiQubit.pairStateVec, // lower
                multiQubit.stateVec); // output
    ...
    ComplexMatrix2 u)
{
    ...
    if (useLocalDataOnly) {
        ...
        // this rank's values are either in the upper of lower half of the block.
        // send values to controlledUnitaryDistributed in the correct order
        if (rankIsUpper) {
            controlledUnitaryDistributed(multiQubit, controlQubit, targetQubit, rot1,
                rot2,
                multiQubit.stateVec, // upper
                multiQubit.pairStateVec, // lower
                multiQubit.stateVec); // output
    ...
    ComplexMatrix2 u)
{
    ...
    QuESTAssert(mask >= 0 && mask <= (1LL << multiQubit.numQubits) - 1, 2,
        __func__);
    QuESTAssert((mask & (1LL << targetQubit)) != (1LL << targetQubit), 3,
        __func__);
    ...
    if (useLocalDataOnly) {
        ...
        // this rank's values are either in the upper of lower half of the block.
        // send values to multiControlledUnitaryDistributed in the correct order
        if (rankIsUpper) {
            multiControlledUnitaryDistributed(multiQubit, targetQubit, mask, rot1,
                rot2,
                multiQubit.stateVec, // upper
                multiQubit.pairStateVec, // lower
                multiQubit.stateVec); // output
...
void sigmaX(MultiQubit multiQubit, const int targetQubit)
{
    ...
    if (useLocalDataOnly) {
        ...
        exchangeStateVectors(multiQubit, pairRank);
        // this rank's values are either in the upper of lower half of the block.
        // sigmaX just replaces this rank's values with pair values
    ...
    const int targetQubit)
{
    ...
    if (useLocalDataOnly) {
        ...
        exchangeStateVectors(multiQubit, pairRank);
        // this rank's values are either in the upper of lower half of the block.
        // send values to controlledNot in the correct order
        if (rankIsUpper) {
    ...
    enum phaseGateType type)
{
    ...
    if (useLocalDataOnly) {
        ...
        rankIsUpper = chunkIsUpper(multiQubit.chunkId, multiQubit.numAmps, targetQubit);
        if (!rankIsUpper)
            phaseGateDistributed(multiQubit, targetQubit, type);
    }
}
...
void hadamard(MultiQubit multiQubit, const int targetQubit)
{
    ...
    if (useLocalDataOnly) {
        ...
        exchangeStateVectors(multiQubit, pairRank);
        // this rank's values are either in the upper of lower half of the block.
        // send values to hadamardDistributed in the correct order
        if (rankIsUpper) {
 ...
 * sum up 2^q values, then skip 2^q values, etc. This function finds if an
 * entire chunk is in the range of values to be skipped
 *
 * @param[in] chunkId id of chunk in state vector
 * @param[in] chunkSize number of amps in chunk
    ...
    int measureQubit)
{
    ...
    // calculate probability by summing over numChunksToSkip, then skipping
    // numChunksToSkip, etc
...
REAL findProbabilityOfOutcome(MultiQubit multiQubit,
    ...
    int outcome)
{
    QuESTAssert(measureQubit >= 0 && measureQubit < multiQubit.numQubits, 2,
        __func__);
    ...
    if (skipValuesWithinRank) {
        ...
        if (!isChunkToSkipInFindPZero(multiQubit.chunkId, multiQubit.numAmps,
                measureQubit)) {
            stateProb = findProbabilityOfZeroDistributed(multiQubit, measureQubit);
        } else
            stateProb = 0;
    }
    MPI_Allreduce(&stateProb, &totalStateProb, 1, MPI_QuEST_REAL, MPI_SUM,
        MPI_COMM_WORLD);
    if (outcome == 1)
    ...
    int outcome)
{
    ...
    if (skipValuesWithinRank) {
        ...
        if (!isChunkToSkipInFindPZero(multiQubit.chunkId, multiQubit.numAmps,
            ...
            if (outcome == 0)
                collapseToOutcomeDistributedRenorm(multiQubit, measureQubit,
                    totalStateProb);
            else
                collapseToOutcomeDistributedSetZero(multiQubit, measureQubit);
        } else {
...
int measureWithStats(MultiQubit multiQubit, int measureQubit, REAL* stateProb)
{
    ...
    if (stateProbInternal < REAL_EPS)
        outcome = 0;
    else if (1 - stateProbInternal < REAL_EPS)
        outcome = 1;
    else {
        // ok. both P(0) and P(1) are large enough to resolve
        // generate random float on [0,1]
        float randNum = genrand_real1();
        if (randNum <= stateProbInternal)
            outcome = 1;
        else
            outcome = 0;
    }
    if (outcome == 0)
        stateProbInternal = 1 - stateProbInternal;

    int skipValuesWithinRank = halfMatrixBlockFitsInChunk(multiQubit.numAmps, measureQubit);
    if (skipValuesWithinRank) {
        collapseToOutcomeLocal(multiQubit, measureQubit, stateProbInternal,
            outcome);
    } else {
        if (!isChunkToSkipInFindPZero(multiQubit.chunkId, multiQubit.numAmps,
                measureQubit)) {
