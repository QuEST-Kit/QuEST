...
693     QuESTAssert(measureQubit >= 0 && measureQubit < multiQubit.numQubits, 2, __fu
      nc__);
694     QuESTAssert((outcome==0 || outcome==1), 10, __func__);
695 
696         REAL totalStateProb=findProbabilityOfOutcome(multiQubit, measureQubit, ou
      tcome);
697     QuESTAssert(fabs(totalStateProb)>REAL_EPS, 8, __func__);
698 
699         int skipValuesWithinRank = halfMatrixBlockFitsInChunk(multiQubit.numAmps,
       measureQubit);
700     if (skipValuesWithinRank) {
701         collapseToOutcomeLocal(multiQubit, measureQubit, totalStateProb, outcome)
      ;
702     } else {
703         if (!isChunkToSkipInFindPZero(multiQubit.chunkId, multiQubit.numAmps, mea
      sureQubit)){
704             // chunk has amps for q=0
705             if (outcome==0) collapseToOutcomeDistributedRenorm(multiQubit, measur
      eQubit, 
706                     totalStateProb);
707             else collapseToOutcomeDistributedSetZero(multiQubit, measureQubit);
708         } else {
709             // chunk has amps for q=1
710             if (outcome==1) collapseToOutcomeDistributedRenorm(multiQubit, measur
      eQubit, 
711                     totalStateProb);
712             else collapseToOutcomeDistributedSetZero(multiQubit, measureQubit);
713         }
714     }
715         return totalStateProb;
716 }
\end{DoxyCode}
...
Definition at line 765 of file qubits\_\-env\_\-mpi.c.
...
765                                                    {
766     printf("!!!\n");
767     printf("QuEST Error in function %s: %s\n", func, errorCodes[errorCode]);
768     printf("!!!\n");
769     printf("exiting..\n");
770     MPI_Abort(MPI_COMM_WORLD, errorCode);
771 }
\end{DoxyCode}
...
Definition at line 719 of file qubits\_\-env\_\-mpi.c.
...
\begin{DoxyCode}
719                                                     {
720     QuESTAssert(measureQubit >= 0 && measureQubit < multiQubit.numQubits, 2, __fu
      nc__);
721     REAL stateProb; 
722     return measureWithStats(multiQubit, measureQubit, &stateProb); 
723 }
\end{DoxyCode}
...
Definition at line 725 of file qubits\_\-env\_\-mpi.c.
...
\begin{DoxyCode}
725                                                                               {
726     QuESTAssert(measureQubit >= 0 && measureQubit < multiQubit.numQubits, 2, __fu
      nc__);
727 
728     int outcome;
729     // find probability of qubit being in state 1
730     REAL stateProbInternal = findProbabilityOfOutcome(multiQubit, measureQubit, 1
      );
731 
732     // we can't collapse to a state that has a probability too close to zero
733     if (stateProbInternal<REAL_EPS) outcome=0;
734     else if (1-stateProbInternal<REAL_EPS) outcome=1;
735     else {
736         // ok. both P(0) and P(1) are large enough to resolve
737         // generate random float on [0,1]
738         float randNum = genrand_real1();
739         if (randNum<=stateProbInternal) outcome = 1;
740         else outcome = 0;
741     } 
742     if (outcome==0) stateProbInternal = 1-stateProbInternal;
743 
744         int skipValuesWithinRank = halfMatrixBlockFitsInChunk(multiQubit.numAmps,
       measureQubit);
745     if (skipValuesWithinRank) {
746         collapseToOutcomeLocal(multiQubit, measureQubit, stateProbInternal, outco
      me);
747     } else {
748         if (!isChunkToSkipInFindPZero(multiQubit.chunkId, multiQubit.numAmps, mea
      sureQubit)){
749             // chunk has amps for q=0
750             if (outcome==0) collapseToOutcomeDistributedRenorm(multiQubit, measur
      eQubit, 
751                     stateProbInternal);
752             else collapseToOutcomeDistributedSetZero(multiQubit, measureQubit);
753         } else {
754             // chunk has amps for q=1
755             if (outcome==1) collapseToOutcomeDistributedRenorm(multiQubit, measur
      eQubit, 
756                     stateProbInternal);
757             else collapseToOutcomeDistributedSetZero(multiQubit, measureQubit);
758         }
759     }
760 
761     *stateProb = stateProbInternal;
762     return outcome;
763 }
\end{DoxyCode}
...
Definition at line 773 of file qubits\_\-env\_\-mpi.c.
...
\begin{DoxyCode}
773                                                               {
774     if (!isValid) exitWithError(errorCode, func);
775 }
\end{DoxyCode}
