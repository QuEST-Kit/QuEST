...
List64 none = list_getEmptyList64();
...
void leftapplyCompMatr(Qureg qureg, int* targets, int numTargets, CompMatr matrix) {
    ...
    bool conj = false;
    bool transp = false;
    localiser_statevec_anyCtrlAnyTargDenseMatr(qureg, none, none, list_getList64(targets, numTargets), matrix, conj, transp);
}
...
void rightapplyCompMatr(Qureg qureg, int* targets, int numTargets, CompMatr matrix) {
    ...
    bool conj = false;
    bool transp = true;
    auto qubits = util_getBraQubits(list_getList64(targets, numTargets), qureg);
...
void leftapplyDiagMatr(Qureg qureg, int* targets, int numTargets, DiagMatr matrix) {
    ...
    bool conj = false;
    qcomp exponent = 1;
    auto qubits = list_getList64(targets, numTargets);
...
void leftapplyQubitProjector(Qureg qureg, int qubit, int outcome) {
    ...
    qreal prob = 1;
    localiser_statevec_multiQubitProjector(qureg, list_getList64({qubit}), list_getList64({outcome}), prob);
}
...
void leftapplyMultiQubitProjector(Qureg qureg, int* qubits, int* outcomes, int numQubits) {
    ...
    qreal prob = 1;
    auto qubitVec = list_getList64(qubits, numQubits);
    auto outcomeVec = list_getList64(outcomes, numQubits);
...
void rightapplyQubitProjector(Qureg qureg, int qubit, int outcome) {
    ...
    qreal prob = 1;
    auto qubitList = list_getList64({util_getBraQubit(qubit,qureg)});
    localiser_statevec_multiQubitProjector(qureg, qubitList, list_getList64({outcome}), prob);
}
...
void rightapplyMultiQubitProjector(Qureg qureg, int* qubits, int* outcomes, int numQubits) {
    ...
    qreal prob = 1;
    auto qubitVec = util_getBraQubits(list_getList64(qubits, numQubits), qureg);
    auto outcomeVec = list_getList64(outcomes, numQubits);
    localiser_statevec_multiQubitProjector(qureg, qubitVec, outcomeVec, prob);
}
