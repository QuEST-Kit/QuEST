...
// https://github.com/aniabrown/QuEST/blob/master/LICENCE.txt for details
...
const char* errorCodes[] = {
    ...
    "Invalid system size. Cannot print output for systems greater than 5 "
    "qubits.", // 7
...
void createMultiQubit(MultiQubit* multiQubit, int numQubits, QuESTEnv env)
{
    ...
    if ((!(multiQubit->stateVec.real) || !(multiQubit->stateVec.imag)) && numAmpsPerRank) {
        printf("Could not allocate memory!");
        exit(EXIT_FAILURE);
    }

    if (env.numRanks > 1 && (!(multiQubit->pairStateVec.real) || !(multiQubit->pairStateVec.imag)) && numAmpsPerRank) {
        printf("Could not allocate memory!");
        exit(EXIT_FAILURE);
    }
    ...
    if (multiQubit.numQubits <= 5) {
        for (rank = 0; rank < multiQubit.numChunks; rank++) {
            if (multiQubit.chunkId == rank) {
                ...
                if (reportRank || rank == multiQubit.numChunks - 1)
                    printf("]\n");
            }
            syncQuESTEnv(env);
        }
        ...
        printf(
            "Error: reportStateToScreen will not print output for systems of more "
            "than 5 qubits.\n");
}
...
#endif
    sprintf(str, "%dqubits_CPU_%dranksx%dthreads", multiQubit.numQubits,
        env.numRanks, numThreads);
}
...
#pragma omp parallel default(none) \
    shared(stateVecSize, stateVecReal, stateVecImag) private(index)
#endif
    {
#ifdef _OPENMP
...
#ifdef _OPENMP
#pragma omp parallel default(none) \
    shared(chunkSize, stateVecReal, stateVecImag, normFactor) private(index)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
...
#ifdef _OPENMP
#pragma omp parallel default(none) \
    shared(stateInd, stateVecSize, stateVecReal, stateVecImag) private(index)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
...
/**
 * Initialise the state vector of probability amplitudes such that one qubit is
 * set to 'outcome' and all other qubits are in an equal superposition of zero
 * and one.
 * @param[in,out] multiQubit object representing the set of qubits to be
 * initialised
...
#ifdef _OPENMP
#pragma omp parallel default(none)                                     \
    shared(chunkSize, stateVecReal, stateVecImag, normFactor, qubitId, \
        outcome) private(index, bit)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
...
/**
 * Initialise the state vector of probability amplitudes to an (unphysical)
 * state with each component of each probability amplitude a unique floating
 * point value. For debugging processes
 * @param[in,out] multiQubit object representing the set of qubits to be
 * initialised
 */
...
#ifdef _OPENMP
#pragma omp parallel default(none) \
    shared(chunkSize, stateVecReal, stateVecImag, chunkOffset) private(index)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
...
{
    if (fabs(u.r0c0.real * u.r0c0.real + u.r0c0.imag * u.r0c0.imag + u.r1c0.real * u.r1c0.real + u.r1c0.imag * u.r1c0.imag - 1) > REAL_EPS)
        return 0;
    if (fabs(u.r0c1.real * u.r0c1.real + u.r0c1.imag * u.r0c1.imag + u.r1c1.real * u.r1c1.real + u.r1c1.imag * u.r1c1.imag - 1) > REAL_EPS)
        return 0;

    if (fabs(u.r0c0.real * u.r0c1.real + u.r0c0.imag * u.r0c1.imag + u.r1c0.real * u.r1c1.real + u.r1c0.imag * u.r1c1.imag) > REAL_EPS)
        return 0;

    if (fabs(u.r0c1.real * u.r0c0.imag - u.r0c0.real * u.r0c1.imag + u.r1c1.real * u.r1c0.imag - u.r1c0.real * u.r1c1.imag) > REAL_EPS)
        return 0;
...
{
    if (fabs(alpha.real * alpha.real + alpha.imag * alpha.imag + beta.real * beta.real + beta.imag * beta.imag - 1) > REAL_EPS)
        return 0;
    else
        return 1;
}
...
{
    if (fabs(sqrt(ux * ux + uy * uy + uz * uz) - 1) > REAL_EPS)
        return 0;
    else
        return 1;
}
...
{
    Vector unitAxis = { 1, 0, 0 };
    controlledRotateAroundAxis(multiQubit, controlQubit, targetQubit, angle,
        unitAxis);
}
...
void compactUnitaryLocal(MultiQubit multiQubit,
    const int targetQubit,
    Complex alpha,
    Complex beta)
{
...
#pragma omp parallel default(none) shared(                               \
    ...
    alphaImag, betaReal, betaImag) private(thisTask, thisBlock, indexUp, \
    indexLo, stateRealUp, stateImagUp,                                   \
    stateRealLo, stateImagLo)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
...
void unitaryLocal(MultiQubit multiQubit,
    const int targetQubit,
    ComplexMatrix2 u)
{
    long long int sizeBlock, sizeHalfBlock;
...
#ifdef _OPENMP
#pragma omp parallel default(none)                                           \
    shared(sizeBlock, sizeHalfBlock, stateVecReal, stateVecImag, u) private( \
        thisTask, thisBlock, indexUp, indexLo, stateRealUp, stateImagUp,     \
        stateRealLo, stateImagLo)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
        ...
        for (thisTask = 0; thisTask < numTasks; thisTask++) {
            ...
            stateVecReal[indexUp] = u.r0c0.real * stateRealUp - u.r0c0.imag * stateImagUp + u.r0c1.real * stateRealLo - u.r0c1.imag * stateImagLo;
            stateVecImag[indexUp] = u.r0c0.real * stateImagUp + u.r0c0.imag * stateRealUp + u.r0c1.real * stateImagLo + u.r0c1.imag * stateRealLo;

            // state[indexLo] = u10  * state[indexUp] + u11 * state[indexLo]
            stateVecReal[indexLo] = u.r1c0.real * stateRealUp - u.r1c0.imag * stateImagUp + u.r1c1.real * stateRealLo - u.r1c1.imag * stateImagLo;
            stateVecImag[indexLo] = u.r1c0.real * stateImagUp + u.r1c0.imag * stateRealUp + u.r1c1.real * stateImagLo + u.r1c1.imag * stateRealLo;
        }
    }
}
 ...
 * and a subset of the state vector with upper and lower block values stored
 * seperately.
 *
 ...
 *  @param[out] stateVecOut array section to update (will correspond to either
 * the lower or upper half of a block)
 */
void compactUnitaryDistributed(MultiQubit multiQubit,
    const int targetQubit,
    Complex rot1,
    Complex rot2,
...
#ifdef _OPENMP
#pragma omp parallel default(none)                                         \
    shared(stateVecRealUp, stateVecImagUp, stateVecRealLo, stateVecImagLo, \
        stateVecRealOut, stateVecImagOut, rot1Real, rot1Imag, rot2Real,    \
        rot2Imag) private(thisTask, stateRealUp, stateImagUp, stateRealLo, \
        stateImagLo)
#endif
    {
#ifdef _OPENMP
 ...
 * the lower or upper half of a block)
 */
void unitaryDistributed(MultiQubit multiQubit,
    const int targetQubit,
    Complex rot1,
    Complex rot2,
    ComplexArray stateVecUp,
...
void controlledCompactUnitaryLocal(MultiQubit multiQubit,
    const int controlQubit,
    const int targetQubit,
    Complex alpha,
    Complex beta)
...
void multiControlledUnitaryLocal(MultiQubit multiQubit,
    const int targetQubit,
    long long int mask,
    ComplexMatrix2 u)
{
...
#pragma omp parallel default(none)                                        \
    shared(sizeBlock, sizeHalfBlock, stateVecReal, stateVecImag, u,       \
        mask) private(thisTask, thisBlock, indexUp, indexLo, stateRealUp, \
        stateImagUp, stateRealLo, stateImagLo)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
...
void controlledUnitaryLocal(MultiQubit multiQubit,
    const int controlQubit,
    const int targetQubit,
    ComplexMatrix2 u)
{
...
#pragma omp parallel default(none)                                           \
    shared(sizeBlock, sizeHalfBlock, stateVecReal, stateVecImag, u) private( \
        thisTask, thisBlock, indexUp, indexLo, stateRealUp, stateImagUp,     \
        stateRealLo, stateImagLo, controlBit)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
...
/** Rotate a single qubit in the state vector of probability amplitudes, given
 * two complex numbers alpha and beta and a subset of the state vector with
 * upper and lower block values stored seperately. Only perform the rotation
 * where the control qubit is one.
 *
 *  @param[in,out] multiQubit object representing the set of qubits
 *  @param[in] targetQubit qubit to rotate
 *  @param[in] controlQubit qubit to determine whether or not to perform a
 * rotation
 *  @param[in] rot1 rotation angle
 *  @param[in] rot2 rotation angle
 *  @param[in] stateVecUp probability amplitudes in upper half of a block
 ...
 * the lower or upper half of a block)
 */
void controlledCompactUnitaryDistributed(MultiQubit multiQubit,
    const int controlQubit,
    const int targetQubit,
    Complex rot1,
    Complex rot2,
 ...
 * the lower or upper half of a block)
 */
void controlledUnitaryDistributed(MultiQubit multiQubit,
    const int controlQubit,
    const int targetQubit,
    Complex rot1,
    Complex rot2,
...
/** Apply a unitary operation to a single qubit in the state vector of
 probability amplitudes, given
 *  a subset of the state vector with upper and lower block values
 stored seperately. Only perform the rotation where all the control qubits
 are 1.
 *
 *  @param[in,out] multiQubit object representing the set of qubits
 *  @param[in] targetQubit qubit to rotate
...
#pragma omp parallel default(none)                                         \
    shared(stateVecRealUp, stateVecImagUp, stateVecRealLo, stateVecImagLo, \
        stateVecRealOut, stateVecImagOut, rot1Real, rot1Imag, rot2Real,    \
        rot2Imag, mask) private(thisTask, stateRealUp, stateImagUp,        \
        stateRealLo, stateImagLo)
#endif
    {
#ifdef _OPENMP
...
#ifdef _OPENMP
#pragma omp parallel default(none)                                        \
    shared(sizeBlock, sizeHalfBlock, stateVecReal, stateVecImag) private( \
        thisTask, thisBlock, indexUp, indexLo, stateRealUp, stateImagUp)
#endif
    {
#ifdef _OPENMP
 ...
 *  stored seperately. This rotation is just swapping upper and lower values,
 * and stateVecIn must already be the correct section for this chunk
 *
 ...
 *  @param[in,out] multiQubit object representing the set of qubits
 *  @param[in] targetQubit qubit to rotate
 *  @param[in] stateVecIn probability amplitudes in lower or upper half of a
 * block depending on chunkId
 *  @param[out] stateVecOut array section to update (will correspond to either
 * the lower or upper half of a block)
 */
void sigmaXDistributed(MultiQubit multiQubit,
    const int targetQubit,
...
#ifdef _OPENMP
#pragma omp parallel default(none)                          \
    shared(stateVecRealIn, stateVecImagIn, stateVecRealOut, \
        stateVecImagOut) private(thisTask)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
...
void controlledNotLocal(MultiQubit multiQubit,
    const int controlQubit,
    const int targetQubit)
{
    long long int sizeBlock, sizeHalfBlock;
 ...
 *  Operate on a subset of the state vector with upper and lower block values
 *  stored seperately. This rotation is just swapping upper and lower values,
 * and stateVecIn must already be the correct section for this chunk. Only
 * perform the rotation for elements where controlQubit is one.
 *
 *  @param[in,out] multiQubit object representing the set of qubits
 *  @param[in] targetQubit qubit to rotate
 *  @param[in] stateVecIn probability amplitudes in lower or upper half of a
 ...
 * the lower or upper half of a block)
 */
void controlledNotDistributed(MultiQubit multiQubit,
    const int controlQubit,
    const int targetQubit,
    ComplexArray stateVecIn,
    ComplexArray stateVecOut)
...
#pragma omp parallel default(none)                          \
    shared(stateVecRealIn, stateVecImagIn, stateVecRealOut, \
        stateVecImagOut) private(thisTask, controlBit)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
 ...
 *  @param[in] stateVecIn probability amplitudes in lower or upper half of a
 * block depending on chunkId
 *  @param[in] updateUpper flag, 1: updating upper values, 0: updating lower
 * values in block
 *  @param[out] stateVecOut array section to update (will correspond to either
 * the lower or upper half of a block)
 */
void sigmaYDistributed(MultiQubit multiQubit,
    const int targetQubit,
    ComplexArray stateVecIn,
    ComplexArray stateVecOut,
    int updateUpper)
...
#pragma omp parallel default(none)                                           \
    shared(stateVecRealIn, stateVecImagIn, stateVecRealOut, stateVecImagOut, \
        realSign, imagSign) private(thisTask)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
...
#ifdef _OPENMP
#pragma omp parallel default(none) shared(                                \
    sizeBlock, sizeHalfBlock, stateVecReal, stateVecImag,                 \
    recRoot2) private(thisTask, thisBlock, indexUp, indexLo, stateRealUp, \
    stateImagUp, stateRealLo, stateImagLo)
#endif
    {
#ifdef _OPENMP
 ...
 * the lower or upper half of a block)
 */
void hadamardDistributed(MultiQubit multiQubit,
    const int targetQubit,
    ComplexArray stateVecUp,
    ComplexArray stateVecLo,
    ComplexArray stateVecOut,
...
#pragma omp parallel default(none)                                         \
    shared(stateVecRealUp, stateVecImagUp, stateVecRealLo, stateVecImagLo, \
        stateVecRealOut, stateVecImagOut, recRoot2,                        \
        sign) private(thisTask, stateRealUp, stateImagUp, stateRealLo,     \
        stateImagLo)
#endif
    {
#ifdef _OPENMP
...
void phaseGateLocal(MultiQubit multiQubit,
    const int targetQubit,
...
#pragma omp parallel default(none)                                         \
    shared(sizeBlock, sizeHalfBlock, stateVecReal, stateVecImag, recRoot2, \
        type) private(thisTask, thisBlock, indexUp, indexLo, stateRealLo,  \
        stateImagLo)
#endif
    {
        if (type == SIGMA_Z) {
...
#pragma omp for schedule(static)
            ...
            printf("Type %d is an invalid phase gate\n", type);
    }
}

void phaseGateDistributed(MultiQubit multiQubit,
    const int targetQubit,
    enum phaseGateType type)
{
    REAL stateRealLo, stateImagLo;
...
#ifdef _OPENMP
#pragma omp parallel default(none)                              \
    shared(stateVecReal, stateVecImag, recRoot2, type) private( \
        thisTask, stateRealLo, stateImagLo)
#endif
    {
        if (type == SIGMA_Z) {
#ifdef _OPENMP
...
/** Measure the total probability of a specified qubit being in the zero state
 * across all amplitudes in this chunk. Size of regions to skip is less than the
 * size of one chunk.
 *
 ...
 */
REAL findProbabilityOfZeroLocal(MultiQubit multiQubit, const int measureQubit)
{
    // ----- sizes
...
#ifdef _OPENMP
#pragma omp parallel                                                                                           \
    shared(numTasks, sizeBlock, sizeHalfBlock, stateVecReal, stateVecImag) private(thisTask, thisBlock, index) \
        reduction(+                                                                                            \
                  : totalProbability)
#endif
    {
...
#pragma omp for schedule(static)
        ...
        for (thisTask = 0; thisTask < numTasks; thisTask++) {
            ...
            totalProbability += stateVecReal[index] * stateVecReal[index] + stateVecImag[index] * stateVecImag[index];
        }
    }
    return totalProbability;
}

/** Measure the probability of a specified qubit being in the zero state across
    ...
    const int measureQubit)
{
    ...
    // ----- temp variables
    long long int
        thisTask; // task based approach for expose loop with small granularity
...
#ifdef _OPENMP
#pragma omp parallel                                               \
    shared(numTasks, stateVecReal, stateVecImag) private(thisTask) \
        reduction(+                                                \
                  : totalProbability)
#endif
    {
...
#endif
        for (thisTask = 0; thisTask < numTasks; thisTask++) {
            totalProbability += stateVecReal[thisTask] * stateVecReal[thisTask] + stateVecImag[thisTask] * stateVecImag[thisTask];
        }
    }
...
#pragma omp parallel for default(none) shared(                           \
    stateVecSize, stateVecReal, stateVecImag) private(index, bit1, bit2) \
    schedule(static)
#endif
    for (index = 0; index < stateVecSize; index++) {
    ...
    int numControlQubits)
{
    ...
    QuESTAssert(numControlQubits > 0 && numControlQubits <= multiQubit.numQubits,
        4, __func__);
    long long int mask = 0;
    for (int i = 0; i < numControlQubits; i++)
        mask = mask | (1LL << controlQubits[i]);
    QuESTAssert(mask >= 0 && mask <= (1LL << multiQubit.numQubits) - 1, 2,
        __func__);
...
#ifdef _OPENMP
#pragma omp parallel default(none) \
    shared(stateVecSize, stateVecReal, stateVecImag, mask) private(index)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
...
/** Update the state vector to be consistent with measuring measureQubit=0 if
 * outcome=0 and measureQubit=1 if outcome=1. Performs an irreversible change to
 * the state vector: it updates the vector according to the event that an
 * outcome have been measured on the qubit indicated by measureQubit (where this
 * label starts from 0, of course). It achieves this by setting all inconsistent
 *  amplitudes to 0 and
 *  then renormalising based on the total probability of measuring
 * measureQubit=0 or 1 according to the value of outcome. In the local version,
 * one or more blocks (with measureQubit=0 in the first half of the block and
 *  measureQubit=1 in the second half of the block) fit entirely into one chunk.
 *
 *  @param[in,out] multiQubit object representing the set of qubits
 *  @param[in] measureQubit qubit to measure
 *  @param[in] totalProbability probability of qubit measureQubit being either
 * zero or one
 *  @param[in] outcome to measure the probability of and set the state to --
 * either zero or one
 */
void collapseToOutcomeLocal(MultiQubit multiQubit,
    int measureQubit,
...
#pragma omp parallel default(none)                                         \
    shared(numTasks, sizeBlock, sizeHalfBlock, stateVecReal, stateVecImag, \
        renorm, outcome) private(thisTask, thisBlock, index)
#endif
    {
        if (outcome == 0) {
            // measure qubit is 0
...
/** Renormalise parts of the state vector where measureQubit=0 or 1, based on
 * the total probability of that qubit being in state 0 or 1. Measure in Zero
 * performs an irreversible change to the state vector: it updates the vector
 * according to the event that the value 'outcome' has been measured on the
 * qubit indicated by measureQubit (where this label starts from 0, of course).
 * It achieves this by setting all inconsistent amplitudes to 0 and then
 * renormalising based on the total probability of measuring measureQubit=0 if
 * outcome=0 and measureQubit=1 if outcome=1. In the distributed version, one
 * block (with measureQubit=0 in the first half of the block and measureQubit=1
 * in the second half of the block) is spread over multiple chunks, meaning that
 * each chunks performs only renormalisation or only setting amplitudes to 0.
 * This function handles the renormalisation.
 *
 *  @param[in,out] multiQubit object representing the set of qubits
 *  @param[in] measureQubit qubit to measure
 *  @param[in] totalProbability probability of qubit measureQubit being zero
 */
REAL collapseToOutcomeDistributedRenorm(MultiQubit multiQubit,
    const int measureQubit,
    const REAL totalProbability)
{
    // ----- temp variables
    long long int thisTask;
    long long int numTasks = multiQubit.numAmps;
...
#ifdef _OPENMP
#pragma omp parallel shared(numTasks, stateVecReal, \
    stateVecImag) private(thisTask)
#endif
    {
#ifdef _OPENMP
#pragma omp for schedule(static)
 ...
 *  Measure in Zero performs an irreversible change to the state vector: it
 * updates the vector according to the event that a zero have been measured on
 * the qubit indicated by measureQubit (where this label starts from 0, of
 * course). It achieves this by setting all inconsistent amplitudes to 0 and
 *  then renormalising based on the total probability of measuring
 * measureQubit=0 or 1. In the distributed version, one block (with
 * measureQubit=0 in the first half of the block and measureQubit=1 in the
 * second half of the block) is spread over multiple chunks, meaning that each
 * chunks performs only renormalisation or only setting amplitudes to 0. This
 * function handles setting amplitudes to 0.
 *
 *  @param[in,out] multiQubit object representing the set of qubits
 *  @param[in] measureQubit qubit to measure
 */
void collapseToOutcomeDistributedSetZero(MultiQubit multiQubit,
    const int measureQubit)
{
    // ----- temp variables
    long long int thisTask;
...
{
    // init MT random number generator with three keys -- time, pid and a hash of
    // hostname for the MPI version, it is ok that all procs will get the same
    // seed as random numbers will only be used by the master process
