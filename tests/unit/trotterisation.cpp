    ...
    SECTION( LABEL_CORRECTNESS ) {
        ...
        */
        ...
        for (int i = 0; i < steps; i++) {
            applyTrotterizedUnitaryTimeEvolution(qureg, hamil, dt, order, reps, permuteTerms);
...
TEST_CASE( "applyTrotterizedImaginaryTimeEvolution", TEST_CATEGORY ) {
    ...
    SECTION( LABEL_CORRECTNESS ) {
        ...
        // Ground state: all qubits align down (driven by strong magnetic field)
        {
            ...
            for (int i = 0; i < steps; ++i) {
                applyTrotterizedImaginaryTimeEvolution(qureg, ising, tau, order, reps, permuteTerms);
