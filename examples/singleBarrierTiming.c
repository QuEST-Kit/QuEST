...
int main (int narg, char** varg) {
	...
	double prob;
	prob= calcTotalProbability(multiQubit);
	...
	//	if (REPORT_TIMING) {
	//		wtime_stop = system_timer();
	//		timingVec[rotQubit]=(wtime_stop-wtime_start)/(double)N_TRIALS;
	//	}
		printf("q=%d. barrier time from rank %d of %d: %.5f. comp time: %.5f\n", 
				rotQubit, env.rank, env.numRanks, barrierStop-barrierStart, wtime_stop-wtime_start);
...
*/
        if (REPORT_TIMING && env.rank==0){
                ...
                for(index=0; index<numQubits; index++){
			avg = timingVec[index];
                        fprintf(timing, "%ld, %.8f, NULL, NULL, NULL, NULL\n", index, avg);
                }
        }
