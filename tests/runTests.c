...
int test_compactUnitary(char testName[200])
{
    ...
    if (passed)
        passed = compareReals(1.0, outcome, COMPARE_PRECISION);
    destroyMultiQubit(mq, env);
...
int test_unitary(char testName[200])
{
    ...
    // assigning alpha/beta values to u such that compactUnitary should match
    // unitary
    if (passed)
        passed = compareStates(mq, mqVerif, COMPARE_PRECISION);

    // Rotate back the other way and check we arrive back at the initial state
    uDagger.r0c0.real = u.r0c0.real;
    uDagger.r0c0.imag = -u.r0c0.imag;
    uDagger.r0c1.real = u.r1c0.real;
    uDagger.r0c1.imag = -u.r1c0.imag;
    uDagger.r1c0.real = u.r0c1.real;
    uDagger.r1c0.imag = -u.r0c1.imag;
    uDagger.r1c1.real = u.r1c1.real;
    uDagger.r1c1.imag = -u.r1c1.imag;
...
int test_findProbabilityOfOutcome(char testName[200])
{
    ...
    for (qubit = 0; qubit < numQubits; qubit++) {
        ...
        if (passed)
            passed = compareReals(1, outcome, COMPARE_PRECISION);

        outcome = findProbabilityOfOutcome(mq, qubit, 1);
        if (passed)
            passed = compareReals(0, outcome, COMPARE_PRECISION);
    }
    ...
    for (qubit = 0; qubit < numQubits; qubit++) {
        ...
        outcome = findProbabilityOfOutcome(mq, qubit, 0);
        if (passed)
            passed = compareReals(0.5, outcome, COMPARE_PRECISION);
...
int test_collapseToOutcome(char testName[200])
{
    ...
    for (qubit = 0; qubit < numQubits; qubit++) {
        ...
        if (passed)
            passed = compareReals(1, prob, COMPARE_PRECISION);
        if (passed)
            passed = compareStates(mq, mqVerif, COMPARE_PRECISION);
    ...
    for (qubit = 0; qubit < numQubits; qubit++) {
        ...
        prob = collapseToOutcome(mq, qubit, 0);
        if (passed)
            passed = compareReals(0.5, prob, COMPARE_PRECISION);
        if (passed)
            passed = compareStates(mq, mqVerif, COMPARE_PRECISION);
...
int test_measureWithStats(char testName[200])
{
    ...
    for (qubit = 0; qubit < numQubits; qubit++) {
        ...
        if (passed)
            passed = (outcome == 0);
        if (passed)
            passed = compareReals(prob, 1, COMPARE_PRECISION);
        if (passed)
            passed = compareStates(mq, mqVerif, COMPARE_PRECISION);
    }
    ...
    for (qubit = 0; qubit < numQubits; qubit++) {
        ...
        outcome = measureWithStats(mq, qubit, &prob);
        if (passed)
            passed = compareReals(prob, 0.5, COMPARE_PRECISION);
    }
    destroyMultiQubit(mq, env);
    destroyMultiQubit(mqVerif, env);
...
int main(int narg, char** varg)
{
    ...
    char testNames[NUM_TESTS][200] = { "controlledNot",
        "initStateZero",
        "initStatePlus",
        "initClassicalState",
        "sigmaX",
