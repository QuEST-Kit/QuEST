"""
Core module of testing facility
"""
...
import math
import random
from QuESTPy.QuESTBase import (fileWarning, fnfWarning, funWarning)
from QuESTPy.QuESTTypes import (list_funcnames, qreal, stringToList, Complex,
                                argComplex)
from QuESTPy.QuESTFunc import (QuESTTestee,
                               createQuESTEnv, reportQuESTEnv, destroyQuESTEnv,
                               createQureg, destroyQureg,
                               initZeroState, initPlusState, initDebugState, initClassicalState, setAmps,
                               setDensityAmps,
                               getDensityAmp, getAmp, getNumAmps,
                               calcProbOfOutcome, calcTotalProb)
...
@total_ordering
class TestSet:
    """ Main class for holding a sortable test set """
    ...
    def names(self):
        """ Return generator of names of tests in set """
        return (os.path.basename(os.path.splitext(test)[0]) for test in self.tests)

class SuperTestSet(list):
    """ Superset containing groups of test sets """
    def __call__(self, target):
        *specs, name = target.split(":")
        if name == "all":
            return SuperTestSet([testSet for testSet in self.depth(1)])

        return SuperTestSet([testSet for testSet in self
                             if testSet.name == name and all((path in testSet.path for path in specs))])

    def depth(self, depth):
        """ Find all tests within depth range """
        if isinstance(depth, int) or depth is None:
            mindepth = maxdepth = depth
        elif len(depth) == 2:
            mindepth, maxdepth = depth
        else:
            raise TypeError('Unexpected type in SuperTestSet.depth')
...
def init_tests(unitTestPath, logFilePath, tolerance=None, quiet=False, mpiLog=False, fullLog=False):
    ...
    unitPath = [path.rstrip('/ ') for path in unitPath]
...
def construct_test_list(path):
    """ Traverse directories to build a list of tests """
    newTestSets = SuperTestSet()
    ...
    def find_tests(direc, depth):
        ...
        for obj in os.listdir(direc):
            ...
            if obj.startswith('__') and obj.endswith('__'): # Skip dunders
                continue
                ...
                if fileType in [".test", ".py"]:
                    tests.append(loc)
                    newTestSets.append(TestSet(filename, depth + 1, direc, [loc]))
...
def find_file(filename, returnPath=False, write=False, core=False, searchPath=None):
    ...
    for path in searchPath:
        ...
        for pathRoot, _, filenames in os.walk(path):
            if filename in filenames:
                if returnPath:
                    return os.path.join(pathRoot, filename)
                else:
                    return QuESTTestFile(os.path.join(pathRoot, filename), fullTest=False)
...
class QuESTTestFile:
    ...
    def init_testfile(self):
        ...
        try:
            ...
            raise IOError(fileWarning.format(
                message='Header of file :\n'+temp+"\n does not contain the number of tests",
    ...
    def read_state_vec(self, numQubits=0, denMat=None):
        ...
        for _ in range(nStates): # Compare final with expected states
            try:
...
class TestResults:
    ...
    def _write_term(self, *out, **kwargs):
        ...
        if self._printToScreen and root:
            print(*out, **kwargs, flush=True)
        ...
        if a.isDensityMatrix and b.isDensityMatrix:
            ...
            for row in range(a.numQubitsRepresented):
                for col in range(b.numQubitsRepresented):
                    ...
                    if not self.compareComplex(aState, bState, tol):
                        return False
    ...
    def compareComplex(self, a, b, tol=None):
        ...
        if tol is None:
            tol = self._tolerance
        if abs(a.real - b.real) > tol or abs(a.imag - b.imag) > tol:
            return False
        return True
    ...
    def _run_test(self, testFunc, testFile):
        ...
        for _ in range(testFile.nTests):
                ...
                for test in testType[0]:
                    if test in "Mm":
                        ...
                        for qubit in range(qubits.numQubitsRepresented):
                            ...
                            success = (success and
                                       self.compareReals(calcProbOfOutcome(qubits, qubit, 0),
                                                         expectState[qubit][0]) and
                                       self.compareReals(calcProbOfOutcome(qubits, qubit, 1),
                                                         expectState[qubit][1]))
                        ...
                        if not success:
                            for qubit in range(qubits.numQubitsRepresented):
                                self.log('{} {}'.format([calcProbOfOutcome(qubits, qubit, 0),
                                                         calcProbOfOutcome(qubits, qubit, 1)], expectState[qubit]))
    ...
    def run_tests(self, testsToRun):
        ...
        for tests in testsToRun:
            for testSet in testSets(tests):
                if not testSet.tests:
                    continue
    ...
    def _write_gen_results(self, outputFile, testGen, qubitOp, result=None, testQubits=None):
        if qubitOp:
            for test in testGen:
                if test in "Pp":
                    outputFile.write("{}\n".format(round(calcTotalProb(testQubits), self._tolOrder+2)))
                elif test in "Ss":
                    for elem in testQubits._state_vec():
                        outputFile.write(str(elem))
                elif test in "Mm":
                    for qubit in range(testQubits.numQubitsRepresented):
                        outputFile.write("{} {}\n".format(round(calcProbOfOutcome(testQubits, qubit, 0),
                                                                self._tolOrder+2),
                                                          round(calcProbOfOutcome(testQubits, qubit, 1),
                                                                self._tolOrder+2)))
                else:
    ...
    def gen_std_test(self, testFunc, testFile, nQubits=3,
                     qubitGen="zpdnZPDR", testGen="PMS", targScan="E", contScan="E"):
        """ Generate individual test for a given function """

        for i in range(1, testFunc.nArgs):
            if testFunc.defArg[i] is None:
                self._write_term(
                    'Unable to generate test for function {} invalid default arguments'.format(testFunc.funcname))
                return
        ...
        while qubitGen.find('*') > -1:
            ...
            while i+j < len(qubitGen) and qubitGen[i+j].isdecimal():
                ...
                j += 1
            if j == 1:
                raise IOError('Repeater not followed by repeat value')
            qubitGen = qubitGen.replace(qubitGen[i-1:i+j], dup*int(mul))

        if testFunc.target:
            if testFunc.targetType == "Qubit":
                if targScan == "E":
                    targScan = list(range(nQubits))
                elif targScan == "D":
                    targScan = testFunc.defArg[testFunc.target]
                else:
                    targScan = map(int, targScan.split(","))
            elif testFunc.targetType == "Index":
                if targScan == "E":
                    targScan = list(range(2**nQubits))
                elif targScan == "D":
                    targScan = testFunc.defArg[testFunc.target]
                else:
                    targScan = map(int, targScan.split(","))
        ...
        with open(testFile, 'w') as outputFile:
            ...
            for target in targScan:
                for qubitType in qubitGen:
                    ...
                    if ((testFunc.denMat is False and denMat is True) or (testFunc.denMat is True and denMat is False)):
                    ...
                    for arg in range(1, testFunc.nArgs):
                        args.append(testFunc.defArg[arg])

                    # Overwrite target qubit
                    if testFunc.target:
                        args[testFunc.target] = target
                    ...
                    if qubitType in "RNrn":
                        ...
                        for elem in args[0]._state_vec():
                            outputFile.write(str(elem).rstrip()+",")
                        outputFile.write("] ")
                    else:
                        outputFile.write("{}-{} {}".format(qubitType, testGen, nQubits))

                    # Write final argslist
                    for arg in range(1, testFunc.nArgs):
                        outputFile.write(" "+str(args[arg]))
    ...
    def gen_tests(self, testsToGen=None, nQubits=None, qubitGen=None, testGen="PMS", targScan="E", contScan="E"):
        """ Generate sets of tests and skip if listed in don't_generate """
        if testsToGen is None:
            testsToGen = ["all"]
...
def argQureg(nBits, qubitType, testFile=None, initBits=None, denMat=None):
    ...
    qubitTypes = {
        "z":initZeroState, "p":initPlusState, "d":initDebugState,
        "c":setAmps, "b":setAmps, "r":setAmps, "n":setAmps,      # State Vector
        "Z":initZeroState, "P":initPlusState, "D":initDebugState,
        "C":setAmps, "B":setAmps, "R":setAmps                    # Density Matrix
    }

    if qubitType not in qubitTypes:
        if testFile is not None:
            raise IOError(fileWarning.format(
                message='Expected qubit state ({}), received {}'.format(",".join(qubitTypes.keys()), qubitType),
    ...
    if qubitType == "B":
        ...
        if nReqStates != nInStates:
            raise IOError(
                fileWarning.format(
                    message='Bad number of states expected {}, received {}'.format(nStates, nInStates),
        ...
        if not denMat:
            ...
            setDensityAmps(qubits, qAmpsReal, qAmpsImag)
