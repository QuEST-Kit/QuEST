#!/usr/bin/env python3
"""
Module for setting up and running the QuESTTest test suite
"""
...
parser = argparse.ArgumentParser(description='Python test suite for the Quantum Exact Simulation Toolkit (QuEST).',
                                 epilog='''NOTE: Tests can be specified as full filepaths or testnames, which will be searched for in the TESTPATH, with the earliest path taking priority.
...
parser.add_argument('-e', '--extension', help="Define QuEST Library Extension. Default=%(default)s", default=".so")
parser.add_argument('-Q', '--questpath', help="Define alternative QuEST library location. This may be a path the directory to be searched or a direct path to the file. The default search location is '<questpath>/libQuEST<extension>'. Default=%(default)s", default=QuESTPath)
...
try:
    ...
    init_QuESTLib(QuESTPath[0].questpath, QuESTPath[0].extension)

    # Import remaining libraries
    from QuESTTest.QuESTCore import (init_tests, list_test_sets, list_all_tests, finalise_tests,
                                     root, testResults, repEnv)
...
testArg = parser.add_argument('tests', nargs=argparse.REMAINDER, metavar="TESTS", help="Set of tests one wishes to run, this can be any, any custom test (see NOTE) or any exposed QuEST function. DEFAULT=unit")
...
init_tests(unitTestPath=argList.testpath,
           logFilePath=argList.logfile,
           tolerance=argList.tolerance,
           quiet=argList.quiet,
           mpiLog=argList.mpilog,
           fullLog=argList.full_log)

# Now we manually handle the print with *all* potential arguments included

# Set default for the tests to run
if not argList.tests:
    argList.tests = ["unit"]
...
if any([argList.generate, argList.list_tests, argList.list_all_tests, argList.help]):
    ...
    if argList.generate:
        ...
        if root:
            testResults.gen_tests(testsToGen=argList.tests,
                                  nQubits=argList.numqubits,
                                  qubitGen=argList.quregtypes,
                                  testGen=argList.checks,
                                  targScan=argList.targets,
                                  contScan=argList.controls)
    quit()
